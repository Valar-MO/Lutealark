...
| 工具 `/tools` | 默认收起的轻计划、专注、环境降载、感官降载和微运动；支持点击展开、离开工具页后显示的应用内全局迷你计时器、折叠专注卡的醒目实时状态，以及 5/10/15/25 分钟预设与 1～90 分钟自定义专注时长 |
...
同步收紧后的 P03 后，Q08 第二次真实 `OUTPUT` 仍返回相同的合格 Metadata 合同和 2 条同次归一化来源，候选分支的 RAG、Metadata 与逐项 grounding 再次通过；正文不再包含呼吸指导、不返回 `action`，并明确允许用户暂停，说明呼吸同意门槛与动作语义问题已经修复。但“戴耳塞或播放白噪音”仍包含两个备选动作。情绪质量规则保留为离线测试与 Prompt 调优工具，不再作为实时聊天拦截器：OpenTrek 返回非空在线回复后会原样交付，`auto` 模式只在网络、超时、网关或无有效内容等连接性故障时才离线降级。平台仍必须通过 Prompt、策略和 `action` Metadata 保证普通 P03 不绕过呼吸确认；呼吸、环境和微运动策略后续可增加各自的 action/同意状态校验器。零召回时的 `ragUsed=false` 回退分支仍是已发布联调版本的验收缺口。
...
聊天界面按 Agent 的原始自然段展示，不会再把任意回复强行切分并标记为“情绪确认、医学解释、缓冲建议、环境微调、下一步邀请”。这避免单一情绪支持分支在视觉上被误解为并行执行多个工作流分支；来源、动作按钮、危机支持与记忆候选仍保持独立展示。`offer_light_plan` 与其他 `offer_*` 一样仅表示邀请，不会显示导航按钮；用户明确确认后由 `open_light_plan` 打开轻计划。

在线回复不展示“OpenTrek 在线”或 RAG 使用状态的技术标签；当存在合格来源时，用户仍可通过“参考来源”查看来源。离线基础支持提示继续保留，以便用户了解当前回复的可用范围。

候选工作流新增 `environment_support`，用于图片环境微调的非 RAG 回复；后端与 Metadata Schema 已保留该意图及其 `ragUsed=false`、`sources=[]` 合同。当前发布版本仍不接收浏览器附件：正式图片上传与向 OpenTrek 转发必须先按平台提供的上传/附件 API 实现，不能把本地 Blob、数据 URL 或任意外部 URL 伪装成平台附件。

对话中的 `open_focus_timer` 会打开工具页并将轻专注计时置顶、展开和高亮；手动进入工具页仍保持默认工具排列。工具页在宽屏使用两条独立卡片流而非统一行高的 Grid，避免高矮卡片互相留下大块空白；触碰或悬停卡片、选中环境建议时有轻微上浮和层次反馈，减少动态偏好会关闭此位移动画。计时器保留 5、10、15、25 分钟预设，并允许输入 1～90 分钟的整数自定义时长；所选时长按当前数据主体和当天保存在本机。

周期状态页的“周期记录与预测”支持以选定日期记录月经开始、月经结束或无异常。月经开始会保存为周期事件、更新最新末次月经日期并重新计算阶段与预测；月经结束和无异常同样保留为事件，但不会擅自改变预测。周期计算结果继续注入对话工作流已有的周期字段。

周期曲线会以末次月经日期、平均周期长度和当前周期日绘制完整一个周期的相对雌激素、孕酮趋势，并将今天与当前阶段定位在曲线中；它随周期记录和长度设置更新。曲线明确标为预测性的相对趋势，不显示或暗示真实激素数值，也不替代检测或医疗判断。
...
- 情绪回复质量规则保留在离线测试中，用于发现 P03 的错误 action、呼吸指导或多个/备选/多步骤建议；它不再阻断实时 OpenTrek 聊天。`auto` 模式只在连接性故障时离线降级，`online` 模式直接交付成功的 OpenTrek 回复。
