...
export function TodayGentlePlan({ items, onChange, onItemCompleted }: TodayGentlePlanProps) {
  ...
  return (
    <section className="rounded-[22px] border border-[#ddd8ce] bg-white/80 p-4 shadow-[0_12px_36px_rgba(73,66,55,.06)] sm:p-5">
      <div className="flex flex-wrap items-end justify-between gap-2">
        ...
        <p className="text-xs text-[#8d877e]">已完成 {completedCount}/{normalizedItems.length} · 最多 {MAX_GENTLE_PLAN_ITEMS} 项</p>
      </div>
      ...
      {normalizedItems.length === 0 ? (
        <div className="mt-4 rounded-2xl border border-dashed border-[#d8d2c8] bg-[#faf9f6] px-4 py-5 text-center text-sm text-[#918b82]">
...
export function FocusTimer({
  ...
  const [durationMinutes, setDurationMinutes] = useState<FocusDurationMinutes>(initialDurationMinutes)
  const completionIdRef = useRef(createToolId())
  const timer = useCountdown(durationMinutes * 60, () => {
    onComplete?.({ id: completionIdRef.current, durationMinutes, completedAt: new Date().toISOString() })
  })
  ...
  return (
    <section className="rounded-[22px] border border-[#ddd8ce] bg-white/80 p-4 shadow-[0_12px_36px_rgba(73,66,55,.06)] sm:p-5">
      <p className="text-[11px] font-medium tracking-[.16em] text-[#7d8d74]">GENTLE FOCUS</p>
      <h2 className="mt-1 font-serif text-xl font-semibold text-[#384035]">轻专注番茄钟</h2>
      <p className="mt-2 text-sm leading-6 text-[#7a756d]">只承诺这一小段时间。时间到后可以停下，不会自动开始下一轮。</p>

      <div className="mt-4 flex gap-2" aria-label="番茄钟时长">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        ...
        <button type="button" onClick={() => timer.reset(durationMinutes * 60)} className="rounded-xl border border-[#d4cec4] bg-white px-4 py-3 text-sm text-[#69645c]">重置</button>
      </div>
    </section>
  )
}
...
function SceneTool<TId extends string>({
  ...
  return (
    <section className="rounded-[22px] border border-[#ddd8ce] bg-white/80 p-4 shadow-[0_12px_36px_rgba(73,66,55,.06)] sm:p-5">
      <p className="text-[11px] font-medium tracking-[.16em] text-[#7d8d74]">{eyebrow}</p>
      <h2 className="mt-1 font-serif text-xl font-semibold text-[#384035]">{title}</h2>
      <p className="mt-2 text-sm leading-6 text-[#7a756d]">{description}</p>

      <div className="scrollbar mt-4 flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label={`${title}场景`}>
      ...
      <div className="mt-3 rounded-2xl bg-[#f6f5f1] p-3 sm:p-4">
        ...
        <ul className="mt-3 space-y-2">
          {scene.suggestions.map((suggestion) => (
            <li key={suggestion} className="flex items-start gap-2 rounded-xl bg-white/85 p-3">
...
export function MicroMovementTool({
  ...
  return (
    <section className="rounded-[22px] border border-[#ddd8ce] bg-white/80 p-4 shadow-[0_12px_36px_rgba(73,66,55,.06)] sm:p-5">
      <p className="text-[11px] font-medium tracking-[.16em] text-[#7d8d74]">MICRO MOVEMENT</p>
      <h2 className="mt-1 font-serif text-xl font-semibold text-[#384035]">一到三分钟微运动</h2>
      ...
      <div className="mt-3 rounded-2xl bg-[#f3f5f0] p-4">
