...
describe("agent service OpenTrek degradation boundary", () => {
  ...
  it.each(["auto", "online"] as const)("delivers an OpenTrek emotion reply unchanged in %s mode", async (mode) => {
    openTrek.runAgent.mockResolvedValueOnce({
      sessionCode: runInput.sessionCode,
    ...
    const { runAgent } = await loadService(mode);
    ...
    expect(reply.sessionCode).toBe(runInput.sessionCode);
    expect(reply.content).toContain("白噪音");
    expect(reply.metadata).toMatchObject({
      mode: "online",
      intent: "emotion_support",
      strategy: "none",
