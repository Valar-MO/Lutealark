...
- Agent 消息按 Agent 原始自然段展示；前端不再将任意回复强制映射为“情绪确认、医学解释、缓冲建议、环境微调、下一步邀请”等固定标签，避免把单一策略路径误呈现为多分支流程。禁用的催促/羞耻化词仍以删除线和许可性替代表达并列显示。
...
`offer_*`（包括 `offer_light_plan`）只表示邀请，不直接显示导航按钮；只有直接请求或确认后得到的 `open_*` 才会打开功能。已设置周期时，对话页还会显示当前阶段、周期日、能量/自评能量和缓冲模式摘要。

`open_focus_timer` 打开工具页时会自动展开并高亮轻专注计时；对应计划、环境和微运动入口也会展开目标工具。手动进入工具页时，五个工具均以紧凑卡片收起，点击明确的“展开/收起”控制切换；悬停仅作轻微上浮和阴影加深，不会意外展开，键盘焦点也有清晰边框。折叠只隐藏内容，不会重置正在运行的计时；轻专注番茄钟收起时会以较大、半粗和更深的文字显示设定分钟数与状态，运行中实时显示剩余时间。开始后，离开工具页时应用内其他页面右下角都会显示全局迷你计时器，提供暂停/继续；工具页本身只显示完整番茄钟，不再重复显示悬浮卡。点击主体会回到工具页并自动展开番茄钟。迷你计时器只在当前 Lutealark 标签页内生效，刷新页面会结束本轮计时，不会覆盖其他网站。专注计时提供 5、10、15、25 分钟预设和 1～90 分钟整数自定义时长，选择按当前数据主体和当天保存在本机。
...
| `auto` | 优先 OpenTrek；网络错误、超时、可重试的 5xx，或 HTTP 200 但没有有效 Session/回复内容时安全降级并返回带单行离线标记的本地回复。业务/4xx 错误不会伪装成功。维护者联网测试模式；新克隆模板默认不用它 |
...
- 收紧 P03 后的 Q08 第二次真实复测仍返回完整 Metadata 合同和 2 条同次来源，候选分支的 RAG、Metadata 与逐项 grounding 再次通过；正文已移除呼吸指导、不返回 `action`，并明确允许暂停，呼吸安全与动作语义问题已修复。但“戴耳塞或播放白噪音”仍是两个备选动作。后端保留情绪质量规则作为离线测试与 Prompt 调优工具，不再将其作为实时聊天的 fail-closed 拦截器：成功的 OpenTrek 回复会原样返回；`auto` 仅在连接、超时、网关或无有效内容等失败时降级，`online` 不会因为该风格规则向前端报错。平台仍需依靠策略路由与 `action` Metadata 保证 P03 不绕过呼吸确认；呼吸、环境和微运动策略可后续增加各自的 action/同意状态校验器。零召回时的非 RAG 回退分支仍待实现。
...
- Q01 和 Q05 已验证本机 Session/问答链路；继续验证 Q02/Q08、多轮动作和账号 `memoryUserId` 隔离。Q08 的两个备选动作仍应作为 Prompt/离线质量评估问题处理，但不会再阻断实时聊天。
...
聊天气泡不展示 OpenTrek 在线状态或 RAG 使用状态等联调技术标签。具备合格来源的回复仍保留“参考来源”入口；离线基础支持提示保留，以清楚区分当前回复范围。

周期曲线按照用户保存的末次月经日期、平均周期长度和当前周期日计算完整一个周期的相对趋势，并把今天定位在该周期中。纵轴仅显示低、中、高的预测性相对趋势，不代表真实激素浓度、检测结果或医疗判断；修改周期记录或周期长度后，完整曲线和当前标记会同步更新。

候选 OpenTrek 工作流中的图片环境微调分支使用 `environment_support` 意图，返回非 RAG Metadata（`ragUsed=false`、`sources=[]`）；后端和工作流 Schema 已识别该意图。当前网页尚未开放图片附件：在获得 OpenTrek 正式上传和附件引用协议前，后端继续拒绝任何非空附件，避免把浏览器本地 Blob、数据 URL 或任意外部 URL 当作平台附件转发。
...
| `006_cycle_events.sql` | 月经开始、月经结束与无异常的周期事件 |
...
后端还提供 API 请求体上限（256KB）、Agent 会话/聊天的进程内限速与并发上限、非法 JSON 的统一错误，以及非允许来源的 fail-closed CORS。Vite 本地代理只接受 `localhost`、`127.0.0.1` 或 `::1`，并仅在浏览器 Origin 与当前页面 Host 完全一致时改写为后端目标 Origin，因此 `5173` 被占用后改用 `5174`、`5175` 等端口仍可聊天，外站、错误端口、错误协议和畸形来源不会因此进入允许列表。它们是本机服务的有限保护，不应理解为跨进程或分布式流量防护。
